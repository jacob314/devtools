...
class InspectorController implements InspectorServiceClient {
  InspectorController({
    ...
    if (isSummaryTree) {
      subtreePanel = InspectorController(
  ...
  InspectorController parentTree;

  InspectorController subtreePanel;
    ...
    if (inlineProperties != null) {
      for (DiagnosticsNode property in inlineProperties) {
        inspectorTree.appendChild(treeNode,
            setupInspectorTreeNode(inspectorTree.createNode(), property, false));
      }
    }
    if (children != null) {
      for (DiagnosticsNode child in children) {
        inspectorTree.appendChild(treeNode,
            setupInspectorTreeNode(inspectorTree.createNode(), child, expandChildren));
      }
    }
  }
