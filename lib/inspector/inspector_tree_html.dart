...
abstract class CanvasPaintEntry extends PaintEntry {
  CanvasPaintEntry(double x) : super(x);
...
class InspectorTreeNodeRenderHtmlBuilder
    implements InspectorTreeNodeRenderBuilder {
  double x = 0;
  TextStyle lastStyle;
  String font;
  String color;
  final List<PaintEntry> _entries = [];
  static final CanvasRenderingContext2D _measurementCanvas =
      new CanvasElement(width: 1, height: 1).context2D;
  ...
  InspectorTreeNodeRender build() {
    return InspectorTreeNodeCanvasRender(_entries, new Size(x, rowHeight));
  }
}

class InspectorTreeNodeCanvasRender extends InspectorTreeNodeRender {
  InspectorTreeNodeCanvasRender(List<PaintEntry> entries, Size size)
      : super(entries, size);

  void paint(CanvasRenderingContext2D context, Rect visible) {
    for (CanvasPaintEntry entry in entries) {
...
class InspectorTreeHtml extends InspectorTree {
  InspectorTreeHtml({
  ...
  @override
  InspectorTreeNode createNode() => new InspectorTreeNodeHtml();
  ...
  void paintRow(
    ...
    final TreeRow row = root?.getRow(index, selection: selection);
    if (row == null) {
      return;
    }
  ...
  void scrollToRect(Rect targetRect) {
    _viewportCanvas.scrolToRect(targetRect);
  }
}
