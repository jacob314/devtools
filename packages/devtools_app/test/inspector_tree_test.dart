  ...
  group('inspector tree animation', () {
    ...
    testWidgets('optimize', (WidgetTester tester) async {
      ...
      inspectorTree.optimizeRowAnimation(
        lastVisibleRange:
            VisibleRange(animatedRows[4].node, animatedRows[5].node),
        currentVisibleRange:
      ...
      inspectorTree.optimizeRowAnimation(
        lastVisibleRange:
            VisibleRange(animatedRows[0].node, animatedRows[4].node),
        currentVisibleRange:
      ...
      expect(
        ...
        equals(
          ...
          '      ├───▼[C]Center (animate in)\n'
          '      │     [T]Text (animate in)\n'
          '      └─▼[A]AppBar (animate in) snapAnimationToEnd\n'
          '          [T]Text (animate in) snapAnimationToEnd\n',
        ),
      );

      inspectorTree.animationDone();
      row = inspectorTree.getCachedRow(1);
      row.node.isExpanded = false;
      inspectorTree.animationDone();
      row.node.isExpanded = true;
      expect(
