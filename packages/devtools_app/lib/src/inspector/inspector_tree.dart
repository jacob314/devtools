...
class AnimatedOffset {
  const AnimatedOffset({@required this.last, @required this.current});
  ...
  double opacity(Animation<double> visibilityAnimation) {
    if (last != null && current != null) return 1.0;
    return last == null
        ? visibilityAnimation.value
        : 1 - visibilityAnimation.value;
  }
}
...
class AnimatedRow {
  ...
  void computeState(AnimatedRow previousRow, AnimatedRow nextRow) {
    _ticks = [];
    if (current != null && last != null) {
    } else {
      final showing = current.ticks != null;
      for (int tick in current != null ? current.ticks : last.ticks) {}
      for (int i = 0; i < current.ticks.length; i++) {}
    }
    if ((current?.lineToParent ?? false) || (last?.lineToParent ?? false)) {
      _lineToParent = AnimatedOffset(
        last: last?.lineToParent ?? false ? last.depth.toDouble() - 1 : null,
        current: current?.lineToParent ?? false
            ? current.depth.toDouble() - 1
            : null,
      );
    } else {
      _lineToParent = null;
