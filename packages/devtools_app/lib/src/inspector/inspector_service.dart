...
import '../globals.dart';
import '../ui/fake_flutter/_fake_flutter.dart';
...
class InspectorService extends DisposableController
    ...
    for (var directory in directories) {
      ...
      if (!directory.startsWith('package:')) {
        ...
        if (libIndex > 0) {
          ...
          _rootPackages.add(
              'package:${parts[libIndex - 1]}/${parts.sublist(libIndex + 1).join('/')}');
        }
        // TODO(jacobr): handle bazel workspaces
      }
    }

    await _updateLocalClasses(directories);
  }

  Future<void> _updateLocalClasses(List<String> directories) async {
    _localClasses.clear();
  ...
  bool _isLocalUri(String uri) {
    if (uri.startsWith('package:')) {
      return _rootPackages.any((root) => uri.startsWith(root));
    }
